...
document.querySelector('form').addEventListener('submit', function(event) {
    ...
    let datos = {
        ...
        servicios: obtenerValoresCheckbox(servicios_seleccionados),
...
function obtenerValoresCheckbox(elementos) {
    let valores = [];
...
function enviarDatosAlBackEnd(datos) {
    ...
    fetch(url, options)
        .then(response => {
            ...
            return response.json(); // Si la respuesta es exitosa, devuelve la promesa de la respuesta en formato JSON
        })
        .then(data => {
